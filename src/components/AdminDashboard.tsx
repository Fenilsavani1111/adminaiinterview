...
import { CandidatePerformanceDetail, exportCandidateReport } from './CandidatePerformanceDetail';
...
export function AdminDashboard() {
  ...
  const { getAdminDashboard, getCandidateById, getPerformanceComparison } = useJobPosts();
  ...
  const [exportingCandidates, setExportingCandidates] = useState<Set<string>>(new Set());
  let ignore = false;
  ...
  const handleExportCandidate = async (candidateId: string) => {
    try {
      ...
      // Get comparison data
      const comparisonData = await getPerformanceComparison(candidateData.jobPostId);

      // Export the report
      await exportCandidateReport(candidateData, comparisonData, (isExporting: boolean) => {
        if (!isExporting) {
              ...
              <div className='bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1'>
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <p className='text-3xl font-bold text-gray-900'>{summaryStates.active_jobs}</p>
                    {summaryStates.total_interview === summaryStates?.active_jobs ? (
                      <div className='flex items-center mt-2'>
                  ...
                  <div>
                    ...
                    <div className='flex items-center mt-2'>
                      ...
                      <span className='text-sm text-blue-600 font-medium'>
                        ...
                        {summaryStates.candidate_monthly_growth?.toFixed(2)}% this month
                      </span>
                    </div>
                  </div>
                          ...
                          <div className='flex items-center justify-between'>
                            <div className='flex items-center space-x-4 px-2'>
                              ...
                              <div className='flex-1'>
                                ...
                                <p className='text-xs text-gray-500'>{item?.JobPost?.company}</p>
                                  ...
                                  <div className='text-center'>
                                    <div
                                      className={`text-3xl font-bold mb-1 ${getScoreColor(
                                        item.categoryPercentage?.overallPercentage ?? 0
                                      )}`}
                                    >
                                      {item.categoryPercentage?.overallPercentage ?? 0}%
                                    </div>
