      ...
      if (response?.token?.length > 0) {
        ...
        if (type === "open") {
          ...
          const whatsappURL = `https://wa.me/${
            whatsAppPopupData?.phone_number
...
Best regards`);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* whatsapp Modal */}
      {whatsAppPopupData.showPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-gray-900 mb-4">
              Share Job Post with Whatsapp
            </h3>

            {(() => {
              const job = processedJobPosts.find(
                (j) => j.id === whatsAppPopupData.jobId
              );
              if (!job) return null;
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        type="text"
                        value={whatsAppPopupData.phone_number}
                        onChange={(e) => {
                  ...
                  {whatsAppPopupData.loading ? (
                    ...
                    <>
                      <button
                        onClick={() => {
                          getJobLink(whatsAppPopupData.jobId, "whatsapp");
                        }}
                        className="w-full px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors"
                      >
                        Submit
